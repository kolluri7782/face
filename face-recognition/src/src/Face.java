...
public class Face {
    ...
    public void load(boolean crop) throws MalformedURLException {
        if (file.toURI().toURL() == null) {
            throw new NullPointerException("File is not initialized.");
        }

        Image im = new ImageIcon(file.toURI().toURL()).getImage();
    ...
    public BufferedImage recognise(BufferedImage im) {
        ...
        for (int i = 0; i < originalpixelsd.length - 1; i++) {
